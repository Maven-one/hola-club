...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.audits.update", [$audit->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('device_in_correct_position') ? 'has-error' : '' }}">
                ...
                <select id="device_in_correct_position" name="device_in_correct_position" class="form-control">
                    <option value="" disabled {{ old('device_in_correct_position', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::DEVICE_IN_CORRECT_POSITION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('device_in_correct_position', $audit->device_in_correct_position) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('at_least_2_spare_till_rolls') ? 'has-error' : '' }}">
                ...
                <select id="at_least_2_spare_till_rolls" name="at_least_2_spare_till_rolls" class="form-control" required>
                    <option value="" disabled {{ old('at_least_2_spare_till_rolls', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::AT_LEAST_2_SPARE_TILL_ROLLS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('at_least_2_spare_till_rolls', $audit->at_least_2_spare_till_rolls) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('more_than_20_hola_club_cards_available') ? 'has-error' : '' }}">
                ...
                <select id="more_than_20_hola_club_cards_available" name="more_than_20_hola_club_cards_available" class="form-control">
                    <option value="" disabled {{ old('more_than_20_hola_club_cards_available', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::MORE_THAN_20_HOLA_CLUB_CARDS_AVAILABLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('more_than_20_hola_club_cards_available', $audit->more_than_20_hola_club_cards_available) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('kazang_poster_visible') ? 'has-error' : '' }}">
                ...
                <select id="kazang_poster_visible" name="kazang_poster_visible" class="form-control" required>
                    <option value="" disabled {{ old('kazang_poster_visible', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::KAZANG_POSTER_VISIBLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kazang_poster_visible', $audit->kazang_poster_visible) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('promo_poster_visible') ? 'has-error' : '' }}">
                ...
                <select id="promo_poster_visible" name="promo_poster_visible" class="form-control">
                    <option value="" disabled {{ old('promo_poster_visible', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::PROMO_POSTER_VISIBLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('promo_poster_visible', $audit->promo_poster_visible) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('hola_club_poster_available') ? 'has-error' : '' }}">
                ...
                <select id="hola_club_poster_available" name="hola_club_poster_available" class="form-control">
                    <option value="" disabled {{ old('hola_club_poster_available', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::HOLA_CLUB_POSTER_AVAILABLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('hola_club_poster_available', $audit->hola_club_poster_available) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('promotions_displayed_on_device') ? 'has-error' : '' }}">
                ...
                <select id="promotions_displayed_on_device" name="promotions_displayed_on_device" class="form-control">
                    <option value="" disabled {{ old('promotions_displayed_on_device', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Audit::PROMOTIONS_DISPLAYED_ON_DEVICE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('promotions_displayed_on_device', $audit->promotions_displayed_on_device) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
