...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fieldOpsRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.field-ops-requests.update", [$fieldOpsRequest->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('device') ? 'has-error' : '' }}">
                ...
                <select id="device" name="device" class="form-control">
                    ...
                    @foreach(App\FieldOpsRequest::DEVICE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('device', $fieldOpsRequest->device) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('fault_reason') ? 'has-error' : '' }}">
                ...
                <select id="fault_reason" name="fault_reason" class="form-control" required>
                    ...
                    @foreach(App\FieldOpsRequest::FAULT_REASON_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('fault_reason', $fieldOpsRequest->fault_reason) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
