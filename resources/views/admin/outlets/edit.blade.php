        ...
        <form action="{{ route("admin.outlets.update", [$outlet->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                <select id="status" name="status" class="form-control" required>
                    ...
                    @foreach(App\Outlet::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $outlet->status) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('area') ? 'has-error' : '' }}">
                ...
                <select id="area" name="area" class="form-control">
                    ...
                    @foreach(App\Outlet::AREA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('area', $outlet->area) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('area'))
                    <em class="invalid-feedback">
            ...
            <div class="form-group {{ $errors->has('province') ? 'has-error' : '' }}">
                ...
                <select id="province" name="province" class="form-control">
                    ...
                    @foreach(App\Outlet::PROVINCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('province', $outlet->province) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('rebate_option') ? 'has-error' : '' }}">
                ...
                <select id="rebate_option" name="rebate_option" class="form-control">
                    <option value="" disabled {{ old('rebate_option', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Outlet::REBATE_OPTION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('rebate_option', $outlet->rebate_option) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
